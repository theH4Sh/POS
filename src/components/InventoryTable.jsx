...
// Tracks the mouse itself so following the cursor only re-renders the tooltip,
// never the table full of rows behind it.
function HoverTooltip({ product }) {
  ...
  return createPortal(
    <div
      ...
      style={{
        top: Math.min(pos.y + 15, window.innerHeight - 180),
        left: Math.min(pos.x + 15, window.innerWidth - 280)
      }}
    >
      <div className="flex items-center gap-2 mb-2 border-b border-white/10 pb-2">
        <div className="w-2 h-2 rounded-full bg-blue-400 animate-pulse"></div>
        <p className="font-black text-blue-100 uppercase tracking-tighter">{product.name}</p>
      </div>
      <p className="text-gray-300 mb-3 leading-relaxed text-xs">
        {product.description ? product.description : <span className="italic text-gray-500">No description available</span>}
      </p>
      <div className="flex justify-between items-center text-[10px] text-white/50 bg-white/5 p-2 rounded-lg">
        <span className="font-bold uppercase">{product.category || "General"}</span>
        {product.formulaName && (
        ...
        <span className="font-mono bg-blue-500/20 text-blue-300 px-2 py-0.5 rounded-full border border-blue-500/30">{product.stock || 0} IN STOCK</span>
      </div>
    </div>,
    document.body
  );
...
// Memoized so state changes in the table (hover, sort clicks) don't re-render every row
const ProductRow = memo(function ProductRow({ p, isAdmin, lowStockThresholds, onEditProduct, onDeleteProduct, onHover }) {
  return (
    <tr
    ...
    >
      {/* Barcode */}
      <td className="px-6 py-5 whitespace-nowrap" onMouseEnter={() => onHover(p)}>
      ...
      {/* Name & Category */}
      <td className="px-6 py-5" onMouseEnter={() => onHover(p)}>
      ...
      {/* Pricing */}
      <td className="px-6 py-5 text-center flex items-center" onMouseEnter={() => onHover(p)}>
      ...
      {/* Actions */}
      <td className="px-6 py-5 text-right whitespace-nowrap" onMouseEnter={() => onHover(null)}>
        {isAdmin && (
...
export default function InventoryTable({ products, onEditProduct, onDeleteProduct, isAdmin = false, lowStockThresholds = { default: 20 }, loading = false }) {
  ...
  // Incremental rendering: keep the DOM small even with thousands of matches
  const [visibleCount, setVisibleCount] = useState(ROWS_PER_PAGE);

  // Reset the cap whenever the underlying list changes (search/filter/sort)
  ...
  const handleHover = useCallback((p) => setHoveredProduct(p), []);
  ...
  return (
    <div className="rounded-2xl border border-gray-100 bg-white overflow-hidden shadow-sm relative border-separate">
      {hoveredProduct && <HoverTooltip product={hoveredProduct} />}
      ...
      <div className="overflow-auto max-h-[600px] custom-scrollbar">
        <table className="w-full text-sm border-collapse">
          <thead className="sticky top-0 z-20">
            <tr className="bg-gray-50/80 backdrop-blur-md border-b border-gray-100">
              ...
              >
                <div className="flex items-center">Barcode <SortIcon sortConfig={sortConfig} columnKey="barcode" /></div>
              </th>
              <th
                className="px-6 py-4 text-left text-xs font-bold text-gray-500 uppercase tracking-widest cursor-pointer group hover:text-blue-600 transition-colors"
                onClick={() => requestSort('name')}
              >
                <div className="flex items-center">Product Information <SortIcon sortConfig={sortConfig} columnKey="name" /></div>
              </th>
              <th
              ...
              >
                <div className="flex items-center justify-center text-center">Price (S/B) <SortIcon sortConfig={sortConfig} columnKey="salePrice" /></div>
              </th>
              <th
                className="px-6 py-4 text-center text-xs font-bold text-gray-500 uppercase tracking-widest cursor-pointer group hover:text-blue-600 transition-colors"
                onClick={() => requestSort('stock')}
              >
                <div className="flex items-center justify-center">Stock <SortIcon sortConfig={sortConfig} columnKey="stock" /></div>
              </th>
        ...
        {!loading && sortedProducts.length > visibleCount && (
          <div className="p-4 flex flex-col items-center gap-2 border-t border-gray-50 bg-gray-50/30">
            <span className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">
              Showing {visibleCount} of {sortedProducts.length} products
            </span>
            <button
              onClick={() => setVisibleCount((c) => c + ROWS_PER_PAGE)}
              className="px-6 py-2 rounded-xl bg-white border border-gray-200 text-xs font-bold text-gray-600 hover:border-blue-300 hover:text-blue-600 shadow-sm transition-all active:scale-95"
            >
              Show {Math.min(ROWS_PER_PAGE, sortedProducts.length - visibleCount)} more
            </button>
          </div>
        )}
      </div>
